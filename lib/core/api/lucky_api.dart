...
class Api {
  ...
  /// get my coupons with optional status and order amount filtering
  /// [status] 0-unused, 1-used, 2-expired. Pass null to fetch all
  /// [orderAmount] is optional, but if provided, it will filter coupons that can be applied to an order of that amount. This is useful for the checkout page to show only applicable coupons.
  static Future<PageResult<UserCoupon>> myCouponsApi({
    int page = 1,
    int pageSize = 20,
    int? status,
    double? orderAmount,
    ...
    if (orderAmount != null && orderAmount > 0) {
      query['orderAmount'] = orderAmount;
    }

    final res = await Http.get(
      '/api/v1/client/coupons/me',
      query: query,
    );

    return parsePageResponse(res, (e) => UserCoupon.fromJson(e));
  }

  /// exchange a coupon using a code, the code is generated by the backend and can be distributed as needed (e.g. for marketing campaigns, customer service compensation, etc.)
  /// code example: "LUCKY2026"
  /// exchange a coupon using a code
  static Future<String> redeemCouponApi(String code) async {
    final res = await Http.post(
  ...
  /// claim a coupon manually by its template ID
  static Future<String> claimCouponApi(String couponId) async {
    final res = await Http.post(
      '/api/v1/client/coupons/claim/$couponId',
    );
  ...
  /// Get list of claimable coupons (those that the user can claim but hasn't claimed yet). This is typically used to show a list of available coupons that the user can choose to claim.
